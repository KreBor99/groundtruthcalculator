...
def calculatelocations(position):
    temp = original
...
def calculatetruths(position):  # returns an array of distances from the subject to A-E
    truths = []
    for i in iterate:
...
print(original)
# print(calculatetruths(position, distance))
print(calculatelocations(original))
'''
import math
import analysis
import plotter
...
def calculatelocations(
        position):  # new locations of points and user are found, then passed to calculatetruths to get angles
    temp = original
    if position == 1:  # performs translation for if camera is at bottom of screen
        for i in iterate:
            temp[i][0] = -1 * (original[i][0] - monitor[0] / 2)
